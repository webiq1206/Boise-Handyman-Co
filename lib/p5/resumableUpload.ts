  ...
  try{
    ...
    if(action==='start'){
      ...
      try{
        ...
        if(existing.length){const t=existing[0].payload as Transfer,name=input.name.replace(/[\u0000-\u001f/\\]/g,'_');if(t.size!==input.size||t.name!==name)throw new DraftError('This file changed. Select it again.',409);return json({chunkSize:SCOPE_CHUNK_SIZE,chunks:t.chunks,complete:Boolean(t.complete),name:t.name});}
