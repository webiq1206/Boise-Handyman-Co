...
try {
  for (const url of urls) {
    ...
    const result = { url, width, id, errors: [], consoleErrors: [], requestFailures: [], sliders: [] };
    ...
    try {
      ...
      await page.evaluate(() => { const images=[...document.images].filter(i=>i.getClientRects().length);images.forEach(i=>i.loading='eager');return Promise.race([Promise.allSettled(images.map(i=>i.decode())),new Promise(r=>setTimeout(r,10000))]); });
      ...
      await page.screenshot({ path: `${out}/${id}.jpg`, type: 'jpeg', quality: 80, fullPage: true });
