...
import { primaryKey, pgTable, text, varchar, timestamp, jsonb, decimal, boolean, index, integer, uniqueIndex, uuid, foreignKey, unique, bigserial } from "drizzle-orm/pg-core";
...
export const p5EstimatorFiles = pgTable("p5_estimator_files", {
  ...
  foreignKey({
    name: "p5_estimator_files_draft_id_fkey",
    columns: [table.draftId],
    foreignColumns: [p5EstimatorDrafts.id],
  }),
...
export const p5EstimatorOutbox = pgTable("p5_estimator_outbox", {
  ...
  foreignKey({
    name: "p5_estimator_outbox_draft_id_fkey",
    columns: [table.draftId],
    foreignColumns: [p5EstimatorDrafts.id],
  }),
  unique("p5_estimator_outbox_draft_id_revision_destination_key").on(
    table.draftId,
    table.revision,
    table.destination,
  ),
